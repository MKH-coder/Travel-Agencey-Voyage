...
export const LoginModal: React.FC = () => {
  ...
  // 1. Handle Google OAuth Login (Popup or Redirect)
  const handleGoogleOAuthPopup = async (forceRedirect = false) => {
    setErrorMsg('');
    setErrorField(null);
    setInfoMsg('');
    setOauthNotice(null);
    const inputEmail = email.trim();
    const res = await loginWithGoogle(inputEmail || undefined, undefined, forceRedirect);
    if (res.error) {
      setErrorMsg(res.error);
      if (res.isDomainUnauthorized || res.isPopupBlocked || !inputEmail) {
        setOauthNotice({
          isDomainUnauthorized: Boolean(res.isDomainUnauthorized),
          isPopupBlocked: Boolean(res.isPopupBlocked),
          hostname: typeof window !== 'undefined' ? window.location.hostname : '',
          suggestedEmail: inputEmail || 'mukundkrishna.h2008@gmail.com'
        });
      }
    } else {
      setOauthNotice(null);
          ...
          <div className="space-y-5">
            ...
            {authMode === 'recover' ? (
              ...
              <div className="space-y-4">
                ...
                {/* 1. Official Google OAuth Button & Redirect Option */}
                <div className="space-y-1.5">
                  <button
                    id="google-oauth-popup-btn"
                    type="button"
                    disabled={isLoading}
                    onClick={() => handleGoogleOAuthPopup(false)}
                  ...
                  <div className="flex items-center justify-between px-1 text-[10px]">
                    <span className="text-slate-400">On mobile or popups blocked?</span>
                    <button
                      id="google-oauth-redirect-btn"
                      type="button"
                      disabled={isLoading}
                      onClick={() => handleGoogleOAuthPopup(true)}
                      className="font-semibold text-sky-600 dark:text-sky-400 hover:underline flex items-center gap-1"
                    >
                      <span>Use Full-Screen Redirect</span>
            ...
            {/* Direct Google Account Fallback Banner */}
            {oauthNotice && (
              <div 
                id="oauth-fallback-banner"
                className="p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-900 dark:text-amber-200 text-xs space-y-2.5 animate-in fade-in"
              >
                <div className="flex items-start gap-2">
                  <Globe className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
                  <div className="leading-snug">
                    <span className="font-bold block text-amber-600 dark:text-amber-400">
                      {oauthNotice.isDomainUnauthorized 
                        ? `Firebase Domain Notice (${oauthNotice.hostname})` 
                        : oauthNotice.isPopupBlocked 
                        ? 'Pop-up Window Blocked' 
                        : 'Google Sign-In Notice'}
                    </span>
                    <span className="text-[11px] text-amber-700 dark:text-amber-300">
                      {oauthNotice.isDomainUnauthorized
                        ? `This deployment domain (${oauthNotice.hostname}) requires authorization in Firebase Console: Authentication > Settings > Authorized domains. In the meantime, you can sign in directly below:`
                        : 'Pop-ups were restricted by your mobile browser. Click "Continue with Full-Screen Redirect" or sign in with your email:'}
                    </span>
                  </div>
                </div>
                ...
                <div className="pt-1 flex flex-col gap-1.5">
                  <button
                    type="button"
                    id="retry-oauth-redirect-btn"
                    disabled={isLoading}
                    onClick={() => handleGoogleOAuthPopup(true)}
                    className="w-full py-2 px-3 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-sm transition-all active:scale-[0.99]"
                  >
                    <ExternalLink className="w-3.5 h-3.5" />
                    <span>Try with Full-Screen Redirect</span>
                  </button>

                  <button
                    type="button"
                    id="quick-google-continue-btn"
                    disabled={isLoading}
                    onClick={async () => {
                      const targetEmail = oauthNotice.suggestedEmail;
                      setEmail(targetEmail);
                      setErrorMsg('');
                      setOauthNotice(null);
                      await loginWithGoogle(targetEmail);
                    }}
                    className="w-full py-2 px-3 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-sm transition-all active:scale-[0.99]"
                  >
                    <Mail className="w-3.5 h-3.5" />
                    <span>Continue with {oauthNotice.suggestedEmail}</span>
                  </button>

                  {oauthNotice.suggestedEmail.includes('mukund') && (
                    <button
                      type="button"
                      id="quick-bypass-redirect-btn"
                      onClick={() => {
                        setRecoveryEmailInput(oauthNotice.suggestedEmail);
                        setBypassCodeInput('');
                        setShowLoginModal(false);
                        setShowBypassModal(true);
                      }}
                      className="w-full py-1.5 px-3 rounded-xl border border-amber-500/40 text-amber-700 dark:text-amber-300 font-semibold text-[11px] flex items-center justify-center gap-1.5 hover:bg-amber-500/15 transition-all"
                    >
                      <Shield className="w-3.5 h-3.5 text-amber-500" />
                      <span>Use Technical Super Admin Bypass Key</span>
                    </button>
                  )}
                </div>
              </div>
            ...
            <div className="pt-3 border-t border-slate-200/60 dark:border-slate-800 space-y-2">
              ...
              <button
                id="trigger-auth-audit-logs-btn"
                type="button"
                onClick={() => setShowAuthAuditModal(true)}
                className="w-full flex items-center justify-between p-2 rounded-xl text-left border border-slate-200 dark:border-slate-700/80 bg-slate-100/80 dark:bg-slate-800/40 hover:bg-slate-200/60 dark:hover:bg-slate-850 transition-all text-xs"
              >
                <div className="flex items-center gap-2">
                  <ShieldAlert className="w-3.5 h-3.5 text-rose-500 shrink-0" />
                  <span className="font-medium text-slate-700 dark:text-slate-300 text-[11px]">
                    AuthAudit Client Diagnostics & Error Logs
                  </span>
                </div>
                <span className="text-[10px] text-sky-600 dark:text-sky-400 font-bold uppercase tracking-wider">Inspect</span>
              </button>
            </div>
